...
"</p><hr><h3>Depth</h3><p>" + `${data.features[i].geometry.coordinates[1]}` + "</p>" + "<hr><h3>Magnitude</h3><p>"+`${data.features[i].properties.mag}</p>`));
    ...
    // Start reading the second dataset and putting all points in platesMarkers
    platesMarkers = []
    d3.json(urlplates, function (data){
    platesMarkers.push(L.geoJSON(data));
    //Create a group for both datasets  
    var earthquakes = L.layerGroup(earthquakesMarkers);
    var plates = L.layerGroup(platesMarkers);
  
     // *info* contains the layers for the points and polygons 
  ...
  // Creating the map and adding the layers
    var myMap = L.map("mapid", {
      center: [37.09, -95.71],
    zoom: 4,
    layers: [earthquakes, plates, dark]
    });
  // Add Control
